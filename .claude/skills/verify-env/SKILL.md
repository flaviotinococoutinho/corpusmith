...
## 2. Smoke de runtime SEM tocar ~/llmwiki

`LLMWIKI_HOME` redireciona TODOS os dados — sempre valide num
descartável:

```bash
export LLMWIKI_HOME=$(mktemp -d)/llmwiki-home
backend/scripts/llmwiki okf bootstrap    # → "bundle criado"
backend/scripts/llmwiki seed             # → seed ok: terms/quotations/facts (+pipelines)
backend/scripts/llmwiki okf lint         # → 0 finding(s), 0 erro(s)
backend/scripts/llmwiki doctor           # → {"ok": true, "counts": {"error": 0, ...}}
backend/scripts/llmwiki epistemics lint  # → N mecanismo(s), 0 finding(s)
```

## 3. Smoke da API (daemon vivo, auth real)

```bash
backend/.venv/bin/python -m llmwiki.daemon &   # → "llmwiki daemon em http://127.0.0.1:8377"
TOKEN=$(python3 -c "import json,os;print(json.load(open(os.environ['LLMWIKI_HOME']+'/state/daemon.json'))['token'])")
curl -s -H "x-llmwiki-auth: $TOKEN" http://127.0.0.1:8377/health/full   # → "ok": true
...
Armadilhas conhecidas: o header é `x-llmwiki-auth` (não Bearer); o corpo
