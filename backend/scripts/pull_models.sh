...
#
# Override explícito: CORPUSMITH_CHAT_MODEL / CORPUSMITH_EMBED_MODEL.
set -euo pipefail

EMBED="${CORPUSMITH_EMBED_MODEL:-nomic-embed-text}"
FRACTION="${CORPUSMITH_MEMORY_FRACTION:-0.6}"
...
if [[ -z "$CHAT" ]]; then
  if ! CHAT=$(pick_chat); then
    ...
    echo "  Force um modelo com CORPUSMITH_CHAT_MODEL=<tag> se quiser tentar." >&2
    CHAT=""
  fi
fi
...
if [[ -x "$HERE/../.venv/bin/python" ]]; then
  "$HERE/../.venv/bin/python" -m corpusmith.cli models || true
fi
