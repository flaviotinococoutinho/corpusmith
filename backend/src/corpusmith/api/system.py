...
aceito por header `x-corpusmith-auth` OU query `?auth=` — o `?auth=` existe
...
def build_app(s: Settings, queue: JobQueue, gov: Governor,
    ...
    app = FastAPI(title="corpusmith", version=VERSION)
    ...
    def auth(request: Request) -> None:
        if request.headers.get("x-corpusmith-auth") == token:
            return
    ...
    def root():
        ...
        return {"service": "corpusmith", "version": VERSION,
    ...
    # Até aqui DiagnoseSystem só era alcançável por `corpusmith doctor`: o app
