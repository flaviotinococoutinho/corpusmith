...
ADRs alegam. Nada aqui toca ~/corpusmith: o bundle sintético vive no
...
def bench_ask(*, n_pages: int = 150, asks: int = 12, seed: int = 7) -> dict:
    ...
    with tempfile.TemporaryDirectory(prefix="corpusmith-bench-ask-") as tmp:
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.verb in ("core", "index"):
        with tempfile.TemporaryDirectory(prefix="corpusmith-bench-") as tmp:
