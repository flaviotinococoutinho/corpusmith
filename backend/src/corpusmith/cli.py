...
handshake (app_support/daemon.json). `corpusmith okf lint` usa EXATAMENTE a
    ...
    if data.get("abstained"):
        ...
        for m in data.get("cold_matches", []):
            print(f"  ❄️ fria compatível: {m['page']} "
                  f"(corpusmith recycle {m['page']})", file=sys.stderr)
        return 1
    if data.get("blocked"):
...
def cmd_bench(s: Settings, args) -> int:
    """Delega ao harness versionado (corpusmith.bench) — mesma fonte."""
