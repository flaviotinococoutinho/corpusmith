"""RustComputeKernel — acelerador via extensão PyO3 `corpusmith_native`
...
def native_module():
    import corpusmith_native
    return corpusmith_native
...
class RustComputeKernel:
    def __init__(self):
        ...
        if info.get("protocol") != PROTOCOL_VERSION:
            raise RuntimeError(
                f"corpusmith_native protocolo {info.get('protocol')} ≠ "
