...
def list_backups(settings: Settings) -> list[dict]:
    out = []
    for path in sorted(_backup_dir(settings).glob("corpusmith-backup-*.zip")):
        try:
...
class CreateBackup(UseCase):
    ...
    def execute(self) -> dict:
        ...
        archive = Path(self._out) if self._out else \
            _backup_dir(self._settings) / f"corpusmith-backup-{stamp}.zip"
    ...
    def execute(self) -> dict:
        ...
        if newer:
            raise ValueError(
                ...
                f"({newer}) — atualize o corpusmith antes de restaurar")
        home = self._settings.home.expanduser()
