...
class PlanAttention(UseCase):
    ...
    def execute(self) -> dict:
        ...
        items = self._review_items() + self._gap_items() + self._inbox_items()
    ...
    # ------------------------------------------------------------ fontes
    def _cost(self, words: int) -> float:
    ...
    def _review_items(self) -> list[dict]:
        ...
        for r in rows:
            ...
            out.append({"kind": "review", "target": r["path"],
                        "value": round(gain, 3),
                        "cost_min": self._cost(int(words)),
                        "reason": f"revisão no ponto de esforço produtivo "
                                  f"(P(recall)={p:.2f} ⇒ ganho "
                                  f"{gain:.2f})"})
        idx.close()
        return out

    def _gap_items(self) -> list[dict]:
        ...
        for doc in reader.iter_concepts():
            ...
            if doc.meta.type == "question":
                ...
                kind, reason = "contested", ("página contestada — resolver "
                                             "a disputa vale mais que ler "
                                             "coisa nova")
            ...
            out.append({"kind": kind, "target": doc.rel_path,
                        ...
                        "cost_min": self._cost(words + 200),
                        "reason": reason})
        return out

    def _inbox_items(self) -> list[dict]:
        ...
        for path in sorted((kb / "raw").rglob("*")):
            ...
            out.append({"kind": "inbox", "target": rel,
                        "value": _GAP_VALUE["inbox"],
                        "cost_min": self._cost(words),
