...
| `policy.alias_conflict` | warn (só lint, corpus) | **RFC-006 V2**: um alias reivindicado por 2+ identidades da MESMA camada de precedência (seeds < reference.db < authority_record). O uso vira `confidence=ambiguous` — não é reescrito, não entra em `page_entities`, não lista no frontmatter, pesa 0.15 no grafo. Alvo: o `authority_record` editável; a mensagem nomeia a edição que resolve (tirar o alias nu × declarar o sentido no canônico) e as páginas de uso. Ausência de finding **não** é vocabulário desambiguado — só vê o que alguém curou |
