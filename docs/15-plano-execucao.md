...
| **G-10** ⚠️ **REABERTA** (auditoria, `docs/17` C18: verificado por mutação que o lint fica verde com contrato obrigatório apagado). **Reatribuída ao PR-0.1** | **`epistemics.toml` não tem gate de versão de registro nem de completude** — quatro PRs prometem `1.1.0 → 1.2.0` e nada quebra se esquecerem; nada lista mecanismos **devidos mas ausentes** (é como "nenhum mecanismo de padrão tem contrato" virou achado de auditoria em vez de item de backlog) | `test_epistemics_toml.py` testa parâmetros, não o conjunto nem `[registry] version` | **F2-PR1** |
