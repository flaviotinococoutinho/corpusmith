...
| **T12** ✅ **RESOLVIDO** (ADR-58) | **A extensão nativa era construída e nunca instalada.** `maturin build` produzia o wheel, ninguém o instalava, e `pytest.importorskip("corpusmith_native")` fazia os testes diferenciais do ADR-39 pularem em **todas** as pernas: a equivalência Rust≈Python — única prova de que o acelerador não muda o significado — jamais era exercitada. Mesmo verde-por-skip da G-2 (ramo `[ml]`) e do ADR-47 (binário empacotado) | 🔴 medido em 2026-09-03, quando o bump do PyO3 quebrou a COMPILAÇÃO: uma quebra de COMPORTAMENTO na fronteira FFI teria passado verde. A CI passa a instalar o wheel e rodar os diferenciais; token em `[gate].ci_enforced` + asserção direta sobre o `ci.yml` (`test_a_extensao_nativa_e_instalada_e_exercitada_pela_ci`), porque o token não guarda a si mesmo — medido por mutação |
...
> primeiro. **Fechar é marcar ✅ na própria linha, no mesmo commit**, com
> o teste ou ADR que fechou — a linha FICA aqui: `corpusmith context`
> conta abertos e fechados lendo esta seção, e mover a linha para fora
> faria o item sumir do mapa em vez de constar como entregue.
...
|---|---|---|---|---|---|
| **Q-1** | **Ficha do conceito no cockpit** (V6 + V3 + V5 com rota) ✅ **ENTREGUE** | Rotas read-only via facade (INV-ARCH-004): `GET /cockpit/sheet?page=`, `/cockpit/stability`, `/cockpit/applications?page=`; a ficha no Explorer ao abrir uma página, com as seis linhas do pitch — as duas que faltavam, **"sob qual lente"** (entidades por `base()`/`sentido()`, nível MENÇÃO→PÁGINA declarado no campo `level` e no `means`) e **"onde diverge"** (o grupo do `contradiction_candidate`/`factual_conflict`, sem dizer qual lado está certo); `cost.how`, `means`, `guarantees[].misinterpretations` e `not_measured` renderizados como conteúdo; bloco "o que menos muda" nos Indicadores lendo `page_stability` persistida com carimbo e frescor. **UM dono do refresh (ADR-59)**: os três caminhos viraram um — quem escreve é o comando de refresh, quem lê passa por `retrieval/projections.py`, e a passada de lint (dono: `ComputeDifficulty`) alimenta DUAS projeções na mesma transação (`page_difficulty` + a nova `page_divergence`; index.db v11). O "job leve no scheduler" foi RECUSADO: seria um terceiro recomputador. `_CONTRATOS`, `contratos_citados` e `composite_components` mudaram no mesmo commit, junto com a suposição do contrato que dizia que a ficha recomputava. **Provado por**: `test_q1_ficha_no_cockpit.py`, `ConceptSheetView.test.tsx`, `test_context_pack::test_nenhuma_linha_da_fila_e_largada_em_silencio`, asserção positiva em `test_pontas_soltas::test_a_superficie_de_estudo_da_re_mira_tem_consumidor`, `test_epistemics_toml::test_concept_sheet_parameters_match_code` — 11 mutações executadas, todas mataram | nada | PR | M-01, M-02 |
