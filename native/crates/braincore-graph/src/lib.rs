//! braincore-graph — CSR + Personalized PageRank + union-find +
...
impl Csr {
    ...
    pub fn from_edges(sources: &[u32], targets_in: &[u32], weights_in: &[f32],
                      n: usize) -> Result<Csr, braincore_types::ComputeError> {
        if sources.len() != targets_in.len()
            || sources.len() != weights_in.len() {
            return Err(braincore_types::ComputeError::InvalidInput(
