//! llmwiki_native — bindings PyO3 do compute plane (ADR-39).
//!
...
use braincore_graph::{brandes, components, ppr, Csr};
use braincore_sketch::{candidate_pairs, hamming, simhash_batch};
use braincore_types::{ComputeError, GRAPH_ALGO_VERSION, PROTOCOL_VERSION,
                      SKETCH_ALGO_VERSION};
...
fn backend_info(py: Python<'_>) -> PyResult<Py<PyDict>> {
    ...
    d.set_item("build", option_env!("LLMWIKI_NATIVE_SHA").unwrap_or(""))?;
...
#[pymodule]
fn llmwiki_native(m: &Bound<'_, PyModule>) -> PyResult<()> {
    m.add_class::<Graph>()?;
