//! llmwiki-native-worker — executável para jobs longos (ADR-39 §9).
//!
...
use braincore_etl::{WorkerManifest, WorkerReport};
use braincore_types::{GRAPH_ALGO_VERSION, PROTOCOL_VERSION,
                      SKETCH_ALGO_VERSION};
...
fn run(manifest: &WorkerManifest) -> Result<(Value, Value), (i32, String)> {
    match manifest.job_type.as_str() {
        ...
        "selfcheck" => {
            ...
            let csr = braincore_graph::Csr::from_edges(
                &[0, 1], &[1, 2], &[1.0, 1.0], 3)
                .map_err(|e| (5, e.to_string()))?;
            let ranked = braincore_graph::ppr(
                &csr, &[0], &[1.0], 0.0, 0.5, 20, 1e-9, 3);
            let total: f64 = ranked.iter().map(|(_, s)| s).sum();
            let sketches = braincore_sketch::simhash_batch(
        ...
        "graph_metrics" => {
            ...
            let csr = braincore_graph::Csr::from_edges(&s, &t, &w, n_nodes)
            ...
            let mut central = braincore_graph::brandes(&csr);
            ...
            let comps = braincore_graph::components(&csr);
...
fn main() {
    ...
    let manifest_path = match args.get(1) {
        Some(p) => p.clone(),
        None => {
            eprintln!("uso: llmwiki-native-worker <manifest.json>");
            std::process::exit(2);
        }
    };
